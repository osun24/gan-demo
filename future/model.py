...
def make_discriminator_model(input_shape=(64, 64, 1)):
    model = tf.keras.Sequential()
...
celeba_dataset = tfds.load(name='celeb_a', split='train')
celeba_dataset = celeba_dataset.map(preprocess_images).batch(BATCH_SIZE).shuffle(BUFFER_SIZE)
...
for epoch in range(EPOCHS):
    for image_batch in celeba_dataset:
